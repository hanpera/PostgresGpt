...
namespace PostgresGpt.ApiService.Services;
...
public class ChatService
{
    ...
    {
        return await _dbService.GetSessionsAsync();
    }
    ...
    public async Task<Session> CreateNewChatSessionAsync()
    {
        ...
        await _dbService.InsertSessionAsync(session);
    ...
    public async Task DeleteChatSessionAsync(string? sessionId)
    {
        ...
        await _dbService.DeleteSessionAndMessagesAsync(sessionId);
    }
    ...
    private async Task<List<Message>> GetChatSessionContextWindow(string sessionId)
    {
        ...
        List<Message> allMessages = await _dbService.GetSessionMessagesAsync(sessionId);
    ...
    public async Task<string> SummarizeChatSessionNameAsync(string? sessionId)
    {
        ...
        List<Message> messages = await _dbService.GetSessionMessagesAsync(sessionId);
    ...
    private async Task<Message> CreateChatMessageAsync(string sessionId, string promptText)
    {
        ...
        await _dbService.InsertMessageAsync(chatMessage);
    ...
    private int GetTokens(string userPrompt)
    {
        ...
        Tokenizer tokenizer = TiktokenTokenizer.CreateForModel("gpt-3.5-turbo");

        return tokenizer.CountTokens(userPrompt);
    ...
    private async Task CachePutAsync(string cachePrompts, float[] cacheVectors, string generatedCompletion)
    {
        ...
        await _dbService.CachePutAsync(cacheItem);
    }
    ...
    {
        await _dbService.CacheClearAsync();
    }
}